...
Semantic Kernel è il framework di orchestrazione LLM utilizzato nel client. Fornisce l'astrazione del _Kernel_ (host del modello e dei _plugin_), il _function calling_ automatico --- è il framework a decidere e auto-invocare i tool in base al ragionamento del modello --- e una catena di filtri che permette di intercettare ogni invocazione di strumento.\
Quest'ultimo meccanismo è stato sfruttato per l'anti-loop, l'osservabilità e il rinnovo del _token_. I tool MCP esposti dal server vengono mappati in funzioni del _kernel_ e resi invocabili dal modello.
