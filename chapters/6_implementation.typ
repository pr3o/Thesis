...
Per la visione di un esempio di definizione di _tool_ MCP vedere @cod:mcp-tool-example.
...
1. `MaxToolRoundsFilter`: un filtro *anti-loop* che termina l'esecuzione al superamento di un numero massimo di round di _tool_, vedi @cod:max-tool-rounds;
...
2. *Ritentativo col _token_ corrente (lato _server_ MCP).* Prima di arrendersi, il _client_ REST del _server_ è configurato con `WithActionOnUnauthorized`: su 401 rilegge dal provider il _token_ più recente disponibile, riconfigura il _bearer_ e ritenta la chiamata una volta. Solo se anche questo tentativo fallisce, il _server_ traduce il 401 in un #gl("sentinel") testuale, `__AUTH_EXPIRED`, poiché il canale verso il _client_ trasporta solo stringhe e non può veicolare eccezioni tipizzate. Vedi @cod:sentinel-401.

3. *Refresh reattivo (lato _client_).* Un filtro di auto-invocazione (`SymposiumAuthRefreshFilter`) riconosce la _sentinel_ nel risultato di un _tool_, richiede un nuovo _token_ ad Agilis attraverso la _pipe_, lo applica al _server_ tramite il _tool_ di _control-plane_ `SetSymposiumToken` e ri-invoca l'operazione una sola volta. Un semaforo serializza i 401 paralleli in un unico _refresh_. Vedi @cod:refresh-reactive.
...
- *Errori di formato nella chiamata dei _tool_.* Il modello durante la chiamata ai _tool_ esposti non rispetta il formato ed i nomi dei parametri che il _tool_ MCP si aspetta, con la conseguente impossibilità di eseguire correttamente l'operazione. In seguito ad un certo numero di errori di questo tipo, l'LLM procede allucinando e restituendo all'utente valori inventati.
- *Errori nei valori dei parametri passati ai _tool_.* Stesso problema del punto precedente, ma con parametri correttamente nominati e formattati: il modello passa valori errati, ad esempio date fuori range o codici risorsa inesistenti.
