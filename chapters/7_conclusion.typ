...
#figure(
  ...
  table(
    ...
    [O01], [Comprendere parte dell'architettura del gestionale Agilis], [Sì],
    [O02], [Progettare e sviluppare un server MCP funzionante], [Sì],
    [O03], [Implementare la comunicazione con il sistema LLM], [Sì],
    [O04], [Realizzare funzionalità di accesso ai dati del gestionale], [Sì],
    [O05], [Documentare il sistema sviluppato], [Sì],
    [O06], [Implementare logiche di analisi dei dati e suggerimento di azioni], [Sì],
    [O07], [Strutturare il codice secondo buone pratiche di sviluppo], [Sì],
    [F01], [Estendere il sistema a nuovi casi d'uso applicativi], [Parzialmente]
  )
...
Grazie all'ottimizzazione estrema dei _tool_ e del contesto, il sistema riesce a operare con un modello quantizzato a 9 miliardi di parametri mantenendo un tasso di allucinazioni estremamente contenuto. Tuttavia, permane un margine fisiologico di errore intrinseco alla taglia del modello.
...
correttamente i _tool_, necessita di istruzioni precise e dettagliate all'interno del _System Prompt_. L'aggiunta di nuovi strumenti renderebbe la dimensione del prompt troppo grande, impedendo a un modello di piccola taglia di muoversi e rispondere in maniera rapida e corretta. L'adozione del modello da 27B risulta pertanto la condizione tecnica necessaria non solo per garantire l'affidabilità, ma anche per permettere l'espansione operativa del sistema.
