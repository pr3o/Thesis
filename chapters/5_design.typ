...
3. *Agilis / XSPMPRIS - Chatbot* (Visual Basic .NET, nodo A) - la #gl("ui") del pannello, la lettura del contesto dello _scheduler_, l'anteprima e l'applicazione delle proposte, l'acquisizione e il _refresh_ del token.
...
5. *Esecuzione REST.* Il server MCP esegue ogni tool chiamando l'endpoint `mcp/*` di Symposium (A #sym.arrow.r C) con il _Bearer_ #gl("jwt"); Symposium interroga il database e restituisce JSON.
...
I dati di autorizzazione nascono nella sessione Agilis dell'operatore già autenticato e fluiscono lungo la catena di processi. Vale una distinzione netta, dettata da un principio di minimizzazione dei segreti: le *credenziali* (la password) restano nel processo Agilis, dove servono solo al login iniziale verso Symposium, e non vengono mai passate ai processi MCP; l'*identità di tenant* (azienda, ditta, nome utente) viene propagata come variabile d'ambiente al client MCP, che la ribadisce al server, il quale la trasforma in _header_ fissi delle chiamate REST, usati da Symposium per il filtro _multi-tenant_ che confina ogni query alla ditta dell'operatore; il *token JWT* @jwt-rfc è l'unico segreto che attraversa i confini di processo, e solo come _Bearer_.\
...
- *#gl("chain-of-responsibility").* I tre filtri di auto-invocazione registrati nel _client_ MCP formano una catena ordinata: `MaxToolRoundsFilter` (anti-loop) è il più esterno, `ToolCaptureFilter` (osservabilità) è intermedio, `SymposiumAuthRefreshFilter` (_refresh_ token) è il più interno. Ciascun filtro decide se inoltrare la chiamata al successivo, modificare il contesto o terminare la catena.
...
#v(1em)
- *Transizione da JSON a _Tool-Calling_ per le proposte.* Inizialmente la creazione delle proposte era demandata interamente al modello, a cui veniva richiesto di scrivere manualmente un blocco JSON testuale nella chat, successivamente parsato dall'interfaccia grafica. Questo approccio risultava fragile. È stato quindi introdotto un _plugin_ specifico che permette di registrare la proposta come se fosse un _tool_ a tutti gli effetti, tramite l'inserimento parametrizzato. Il _tool_ associato verifica programmaticamente l'assenza di conflitti in agenda, scartando a monte le proposte invalide. Questo allevia il carico sull'LLM, rendendo il flusso più fluido in quanto il modello non deve più preoccuparsi della formattazione manuale del JSON.
