...
La superficie esposta copre l'intero dominio della pianificazione --- risorse e competenze, task di commessa, calendario lavorativo dei turni, appuntamenti --- affiancata da _endpoint_ di area gestionale (documenti, ordini, offerte, scadenze, anagrafica, articoli) e da pochi _endpoint_ di scrittura (CreaDocumento, CreaOfferta). 
...
La validazione delle richieste entranti è affidata al _middleware_ `SympAuthorizeMiddleware`, che intercetta tutte le comunicazioni prima che qualsiasi _controller_ venga istanziato. Il _middleware_ opera secondo i seguenti passaggi:
...
Il server MCP è costruito con `Host.CreateApplicationBuilder` e l'ordine delle fonti di configurazione è deliberato: le variabili d'ambiente prevalgono su `appsettings.json`, perché il _token_ aggiornato arriva dall'host come variabile d'ambiente e non deve essere sovrascritto da un eventuale _token_ di sviluppo --- scaduto --- presente nel file di configurazione.
In avvio vengono registrati il _provider_ del _token_ come _singleton_ e i tool che espongono le risorse del gestionale; il trasporto è `stdio`.
...
=== Architettura e tassonomia dei Tool
Ogni tool è una classe con un metodo annotato `[McpServerTool]` e `[Description(...)]` che riceve via _dependency injection_ il _resolver_ del client REST.
Il server espone due tipologie di strumenti.
La maggior parte sono tool *pass-through*, che mappano direttamente una risorsa del dominio (appuntamenti, task, risorse, competenze, calendario lavorativo): costruiscono i parametri opzionali, chiamano l'_endpoint_ `mcp/<Risorsa>` e ne formattano la risposta.
Gli altri sono tool *compositi*, che non restituiscono una risorsa grezza ma un risultato derivato, calcolato in modo deterministico aggregando più fonti --- è il caso degli spazi liberi di una risorsa e della ricerca della risorsa disponibile, dettagliati nella @sez:strategia-anti-allucinazione-implementazione.
Fa eccezione, infine, il tool di _control-plane_ per l'aggiornamento del _bearer_, riservato all'host ed escluso dai tool visibili al modello. La tabella completa dei tool e degli _endpoint_ corrispondenti è in #link(<cap:B-appendice>)[Appendice B].

Per la visione di un esempio di definizione di tool MCP vedere @cod:mcp-tool-example.

== Il client MCP: orchestrazione e catena di filtri
Il client MCP è l'host dell'LLM e il coordinatore dell'intera conversazione. In avvio crea il _Kernel_ di Semantic Kernel puntato all'_endpoint_ remoto, avvia il server MCP ereditando le variabili d'ambiente di autenticazione, mappa i tool MCP in funzioni del _Kernel_ --- escludendo il tool di _control-plane_ --- e registra i filtri di auto-invocazione.
...
1. `MaxToolRoundsFilter`: un filtro *anti-loop* che termina l'esecuzione al superamento di un numero massimo di round di tool, vedi @cod:max-tool-rounds;
2. `ToolCaptureFilter`: un filtro di *osservabilità* che accoda nome, parametri e risultato di ogni invocazione in una coda concorrente (`ConcurrentQueue`) a beneficio dell'interfaccia UI, vedi @cod:tool-capture;
...
- *Contenimento dell'esposizione e del contesto.* Dei numerosi _endpoint_ disponibili in Symposium sono stati resi invocabili unicamente gli otto tool _core_ legati alla pianificazione (cfr. @tab:tools), riducendo la superficie decisionale del modello.

=== _Plugin_ nativi: Knowledge e WebFetch
Oltre ai tool MCP remoti, due _plugin_ Semantic Kernel nativi arricchiscono il contesto senza dipendere da Symposium:
...
- *Estrazione degli allegati.* Per soddisfare i requisiti di arricchimento del contesto tramite file, un servizio dedicato (`AttachmentExtractionService`) normalizza documenti eterogenei in testo. Il servizio funge da dispatcher verso estrattori specializzati, selezionati per estensione: i fogli Excel vengono convertiti in tabelle _Markdown_ tramite `DevExpress.Spreadsheet` (con un tetto di righe per foglio); i PDF vengono estratti con `DevExpress.Pdf`; le email sono gestite nel formato `.msg` tramite la libreria _MsgReader_ e nel formato `.eml` tramite _MimeKit_, producendo un'intestazione strutturata (mittente, destinatari, data, oggetto) seguita dal corpo ripulito dalle citazioni. Per contenere l'occupazione di contesto a circa 50K _token_ massimo per turno (empiricamente sufficiente per 5 allegati + contesto visuale + storia conversazionale), ogni allegato è troncato a circa 16K caratteri (circa 4K _token_ per allegato). Il numero massimo di allegati è limitato a 5 per turno.\ Una limitazione nota riguarda i PDF privi di livello testuale (documenti acquisiti come immagine): non essendo previsto un livello di riconoscimento ottico dei caratteri, l'estrazione restituisce un esito vuoto e l'allegato viene marcato in errore con un messaggio esplicativo. Ogni allegato attraversa infatti un proprio ciclo di vita (`InEstrazione`, `Pronto`, `Errore`, `NonSupportato`) e solo gli allegati nello stato `Pronto` concorrono alla costruzione del _payload_ inviato al modello.
...
Il componente centrale è `XspmprisChangesetApplier`, che applica un _changeset_ --- un insieme di operazioni di creazione, modifica ed eliminazione --- agli oggetti già presenti in memoria nel _ViewModel_ della timeline.

Ogni elemento toccato viene marcato come proposto dal chatbot (`IsProposedByChatbot`) e collegato all'identificativo del _changeset_, così da essere distinguibile dal programma e ripristinabile in blocco.
...
Per non perdere lo stato originale si applica il pattern *Memento*, salvando uno snapshot pre-modifica (`PreChatbotSnapshot`) prima di mutare l'oggetto; il valore "pulito" di `HasChanges` viene catturato prima della clonazione e riassegnato per ultimo. Vedi @cod:changeset-memento.


La conferma non è un'azione speciale dell'AI: `ConfermaAnteprima` si limita a scartare lo _snapshot_ di _rollback_ e a ripulire i _marker_, lasciando `HasChanges = True` affinché la modifica venga persistita dal normale flusso di salvataggio di Agilis. L'annullamento (`AnnullaAnteprima`) percorre la strada inversa: ripristina gli `Update` dal `PreChatbotSnapshot`, rimuove le creazioni (riconoscibili dal `Progr` temporaneo negativo) e reintegra le eliminazioni dalla collezione `DeletedItems`.
...
#v(1em)
1. *Margine di scadenza con _fallback_(lato Agilis).* Il connettore verso Symposium rinnova il _token_ *prima* della sua invalidazione, senza attendere l'errore 401, sfruttando il _refresh token_. Se anche il _refresh token_ risulta scaduto --- caso tipico dopo lunghe inattività --- il connettore non propaga l'errore: azzera la sessione e ricade su un _login_ completo con le credenziali ancora custodite nel processo Agilis, ripristinando in modo trasparente una sessione valida. Vedi @cod:auth-margin.

2. *Ritentativo col _token_ corrente (lato server MCP).* Prima di arrendersi, il client REST del server è configurato con `WithActionOnUnauthorized`: su 401 rilegge dal provider il _token_ più recente disponibile, riconfigura il _bearer_ e ritenta la chiamata una volta. Solo se anche questo tentativo fallisce, il server traduce il 401 in un #gl("sentinel") testuale, `__AUTH_EXPIRED`, poiché il canale verso il client trasporta solo stringhe e non può veicolare eccezioni tipizzate. Vedi @cod:sentinel-401.

3. *Refresh reattivo (lato client).* Un filtro di auto-invocazione (`SymposiumAuthRefreshFilter`) riconosce la _sentinel_ nel risultato di un tool, richiede un nuovo _token_ ad Agilis attraverso la _pipe_, lo applica al server tramite il tool di _control-plane_ `SetSymposiumToken` e ri-invoca l'operazione una sola volta. Un semaforo serializza i 401 paralleli in un unico _refresh_. Vedi @cod:refresh-reactive.

4. *_Refresh_ proattivo tra turni e* 5. *unico punto di verità.* Tra un turno e l'altro Agilis può inviare al client un _token_ aggiornato, propagato al server con lo stesso tool di _control-plane_; il `SymposiumTokenProvider`, *singleton* _thread-safe_ nel server MCP, custodisce l'unica copia mutabile del _bearer_ corrente.
...
Per ciascuna di queste quattro funzionalità sono stati identificati 5 scenari realistici, per un totale di 20 situazioni di prova distinte.
Ogni scenario è stato eseguito 10 volte per ogni modello preso in considerazione, per un totale di 200 test complessivi sul modello *Qwen3.5-9B* e 200 test sul modello *Qwen3.6-27B*.
...
Un'esecuzione è stata classificata come _"errata"_ al verificarsi di almeno una delle seguenti quattro condizioni:
#v(1em)
...
- *Errore nella generazione del JSON di proposta.* Questo solo precedentemente all'introduzione del _plugin_ `pianifica_e_proponi`, che ha eliminato la necessità di generare manualmente il JSON di proposta.
#v(1em)
