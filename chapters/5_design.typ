...
- *Adapter.* Il `ChatbotPanelViewModel` non dipende mai direttamente dalle classi concrete del modulo `XSPMPRIS`, ma solo da interfacce: `IPlanningContextProvider` per la lettura del contesto dallo _scheduler_, `IChangesetApplier` per l'applicazione e il _rollback_ dell'anteprima, `IAttachmentExtractionService` per l'estrazione del testo dagli allegati, `IChatbotOrchestratorClient` per la comunicazione col client MCP. Questo disaccoppiamento isola il pannello dai dettagli interni del gestionale e rende i componenti sostituibili e testabili indipendentemente.
- *Decorator.* Il `DeferredOrchestrator` (il cui codice è in #link(<cap:B-appendice>)[Appendice B]) implementa `IChatbotOrchestratorClient` e avvolge l'orchestratore reale: all'apertura del pannello avvia la costruzione dell'orchestratore in un _task_ di _background_, restituendo immediatamente il controllo al _thread_ di interfaccia. Quando arriva il primo messaggio, il `DeferredOrchestrator` attende (`await`) il completamento della _factory_ e inoltra la richiesta all'orchestratore ormai pronto, senza aver mai bloccato la UI.
- *Chain of Responsibility.* I tre filtri di auto-invocazione registrati nel _client_ MCP formano una catena ordinata: `MaxToolRoundsFilter` (anti-loop) è il più esterno, `ToolCaptureFilter` (osservabilità) è intermedio, `SymposiumAuthRefreshFilter` (_refresh_ token) è il più interno. Ciascun filtro decide se inoltrare la chiamata al successivo, modificare il contesto o terminare la catena.
- *Memento.* Prima di applicare una modifica a un appuntamento esistente, il `XspmprisChangesetApplier` ne salva un clone (`PreChatbotSnapshot`) comprensivo del valore originale di `HasChanges`. In caso di annullamento, lo _snapshot_ viene ripristinato integralmente, restituendo l'oggetto allo stato pre-anteprima senza effetti collaterali sul _dirty tracking_ del gestionale.
- *Macchina a stati.* Tre automi a stati finiti governano punti distinti del sistema: il `PanelMode` (cfr. @sez:changeset-anteprima) disciplina gli stati dell'interazione utente-chatbot (`Idle`, `AwaitingResponse`, `Browsing`, `Selected`, `Previewing`, `Refining`); lo _stripper_ dello _stream_ SSE gestisce i marcatori ` thinking` eventualmente spezzati tra _chunk_; il ciclo di vita degli allegati (`InEstrazione`, `Pronto`, `Errore`, `NonSupportato`) isola il _payload_ del modello dai fallimenti di estrazione.
