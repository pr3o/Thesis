...
- *Nodo C - Server applicativo Agilis.* Ospita lo strato dei servizi REST di Symposium, il _plugin_ MCP e il database del gestionale.
#v(1em)
I flussi sono rigidamente direzionali. Il nodo A è il fulcro: apre un flusso HTTP verso il nodo B per inviare i _prompt_ e ricevere lo _streaming_ dei _token_, e interroga in parallelo il nodo C tramite chiamate REST per recuperare i dati gestionali. I nodi B e C non comunicano mai direttamente tra loro, garantendo che il _back-end_ aziendale resti isolato dall'infrastruttura AI.
...
2. *OmegaGruppo.McpClient* (.NET 10 + Semantic Kernel, nodo A) - è l'host dell'LLM e l'orchestratore conversazionale: avvia il server MCP, espone una _Named Pipe_ verso Agilis e dialoga con l'LLM remoto.\ La comunicazione tra Agilis e Omegagruppo.McpClient avviene, appunto, tramite _Named Pipe_ locale, riducendo l'_overhead_ di rete e mantenendo l'isolamento dei processi. La _pipe_ è denominata `OmegaChatbotPipe_{PID}_{GUID:N}` dove `PID` è l'identificativo del processo Agilis e `GUID` un identificativo univoco generato all'apertura del pannello, garantendo che più sessioni simultanee possano coesistere sulla stessa workstation senza conflitti.
3. *Agilis / XSPMPRIS - Chatbot* (Visual Basic .NET, nodo A) - la #gl("ui") del pannello, la lettura del contesto dello _scheduler_, l'anteprima e l'applicazione delle proposte, l'acquisizione e il _refresh_ del _token_.
4. *Symposium - _Plugin_ MCP* (ASP.NET Core, nodo C) - il _back-end_ REST `mcp/*` che interroga il database del gestionale.
#v(1em)
...
5. *Esecuzione REST.* Il server MCP esegue ogni tool chiamando l'_endpoint_ `mcp/*` di Symposium (A #sym.arrow.r C) con il _Bearer_ #gl("jwt"); Symposium interroga il database e restituisce JSON.
6. *Produzione della risposta e cattura delle proposte.* L'LLM produce in _streaming_ il testo visibile e il ragionamento (_think_) su un canale separato, abilitato dal _flag_ `think` nel _payload_ della richiesta. A differenza degli approcci classici basati sul _parsing_ di un JSON generato a fine risposta, il sistema cattura le alternative di pianificazione in modo reattivo: il modello le dichiara invocando un tool dedicato alla proposta, che funge da validatore deterministico.
...
== Sicurezza e ciclo di vita del _token_ <sez:sicurezza-token>
I dati di autorizzazione nascono nella sessione Agilis dell'operatore già autenticato e fluiscono lungo la catena di processi. Vale una distinzione netta, dettata da un principio di minimizzazione dei segreti: le *credenziali* (la password) restano nel processo Agilis, dove servono solo al login iniziale verso Symposium, e non vengono mai passate ai processi MCP; l'*identità di tenant* (azienda, ditta, nome utente) viene propagata come variabile d'ambiente al client MCP, che la ribadisce al server, il quale la trasforma in _header_ fissi delle chiamate REST, usati da Symposium per il filtro _multi-tenant_ che confina ogni query alla ditta dell'operatore; il *_token_ JWT* @jwt-rfc è l'unico segreto che attraversa i confini di processo, e solo come _Bearer_.\

La scadenza del JWT durante una conversazione --- la sessione usa _token_ a tempo --- è gestita con una difesa stratificata, articolata su più livelli: un margine di scadenza che rinnova il _token_ poco prima della sua effettiva invalidazione senza attendere l'errore 401; una rilettura del provider che, su 401, ritenta con il _token_ più recente già disponibile; un _refresh_ reattivo che, riconosciuto un apposito segnale di _token_ scaduto, ne richiede uno nuovo ad Agilis e ritenta l'operazione; un _refresh_ proattivo che aggiorna il _token_ tra un turno e l'altro; e un unico punto di verità che custodisce il _bearer_ corrente. La meccanica di dettaglio di questi livelli è descritta nella @cap:implementazione.
...
- *#gl("chain-of-responsibility").* I tre filtri di auto-invocazione registrati nel client MCP formano una catena ordinata: `MaxToolRoundsFilter` (anti-loop) è il più esterno, `ToolCaptureFilter` (osservabilità) è intermedio, `SymposiumAuthRefreshFilter` (_refresh_ _token_) è il più interno. Ciascun filtro decide se inoltrare la chiamata al successivo, modificare il contesto o terminare la catena.
- *#gl("macchina-a-stati").* Tre automi a stati finiti governano punti distinti del sistema: il `PanelMode` (cfr. @sez:changeset-anteprima) disciplina gli stati dell'interazione utente-chatbot (`Idle`, `AwaitingResponse`, `Browsing`, `Selected`, `Previewing`, `Refining`); lo _stripper_ dello _stream_ #gl("sse") gestisce i marcatori ` thinking` eventualmente spezzati tra _chunk_; il ciclo di vita degli allegati (`InEstrazione`, `Pronto`, `Errore`, `NonSupportato`) isola il _payload_ del modello dai fallimenti di estrazione.

Il catalogo completo dei pattern, con la relativa motivazione, è riportato in #link(<cap:A-appendice>)[Appendice A].

== Decisioni architetturali e _trade-off_
Le scelte di fondo del progetto possono essere lette come una sequenza di compromessi consapevoli. La separazione in tre processi sul nodo A isola il runtime .NET 10 dal processo _legacy_, a fronte dell'impossibilità di mantenere modello e orchestratore in-process. La collocazione dell'LLM su un server remoto centralizza GPU e modello e mantiene leggera la workstation, scartando l'opzione del modello locale per il costo hardware su ogni macchina. L'accesso ai dati esclusivamente via REST mantiene i dati dietro le porte già presidiate, evitando di duplicare logica e di scavalcare la sicurezza con un accesso diretto al database. Il principio per cui l'AI non scrive mai sul database, applicando i _changeset_ come anteprima in memoria, scarta la scrittura diretta come rischio inaccettabile su un gestionale di produzione. Lo _streaming_ a canali separati privilegia una #gl("ux") reattiva rispetto a un'unica risposta a fine elaborazione. Il _refresh_ del _token_ a più livelli garantisce robustezza alla scadenza del JWT senza interrompere la conversazione. La costruzione _lazy_ dell'orchestratore evita il _deadlock_ all'apertura del pannello. Infine, il contenimento del contesto alla sola _viewport_ evita che il modello ragioni su migliaia di record fuori vista, contenendo costo di _token_ e rumore informativo. La tabella sinottica di queste decisioni, con le alternative scartate, è riportata in #link(<cap:A-appendice>)[Appendice A].
...
#v(1em)
- *Transizione da JSON a _Tool-Calling_ per le proposte.* Inizialmente la creazione delle proposte era demandata interamente al modello, a cui veniva richiesto di scrivere manualmente un blocco JSON testuale nella chat, successivamente parsato dall'interfaccia grafica. Questo approccio risultava fragile. È stato quindi introdotto un _plugin_ specifico che permette di registrare la proposta come se fosse un tool a tutti gli effetti, tramite l'inserimento parametrizzato. Il tool associato verifica programmaticamente l'assenza di conflitti in agenda, scartando a monte le proposte invalide. Questo allevia il carico sull'LLM, rendendo il flusso più fluido in quanto il modello non deve più preoccuparsi della formattazione manuale del JSON.
- *Tool ad alta granularità.* Sono stati sviluppati tool complessi (es. calcolo delle disponibilità) che restituiscono risultati finiti, evitando che il modello debba comporre mentalmente più strumenti elementari e dedurne le intersezioni temporali.
